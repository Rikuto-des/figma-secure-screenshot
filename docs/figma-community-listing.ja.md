...
Works with all Figma plans including Dev plan (Dev Mode) — all tools are read-only and fully available.
...
Dev プラン (Dev Mode) でも利用可能です。すべてのツールは読み取り専用で、どのプランでもそのまま動作します。
...
カバー画像は `assets/community/html/cover.html` で正確な文字を重ねてPNG化しています。プレビュー画像もHTML/CSSソースから生成し、画像内テキストの崩れを避けています。
