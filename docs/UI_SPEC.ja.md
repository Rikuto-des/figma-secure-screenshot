...
| `update-selection` | 選択ノードを `root` で更新（spec と選択の**型一致が必要**） | frame=自身のprops＋**子を index＋型で冪等に差分反映** / instance=props / text |
...
- `update-selection` の instance は `componentId` を**無視**して選択インスタンスに props を
  適用する（コンポーネントの差し替えは行わない）。
- `update-selection` の frame：`children` を**省略すると既存の子は据え置き**（他フィールドと
  同じ部分更新）。`children` を**指定すると差分反映**し、spec に無い既存子は削除される
  （`[]` を渡せば全削除）。
...
| `target` `update-selection` の子の差分反映（index＋型で冪等） | ✅ E-2 |
| `instance` の `props`：`INSTANCE_SWAP` | ⏳ 未対応（明確にエラー） |
| チームライブラリ部品（`importComponentByKeyAsync`） | ⏳ 後続 |
| `update-selection` でのコンポーネント差し替え | ⏳ 後続 |
| 子の照合を name ベースに（現状は index＋型） | ⏳ 後続 |

---

**次の一歩：** 動作中の Figma で全モード（create / into-selection / update-selection）を
`validateOnly` → 本適用で e2e 検証する。残機能は INSTANCE_SWAP・チームライブラリ・
コンポーネント差し替え・name ベース照合。
